...
Этот репозиторий демонстрирует практический подход к автоматизации тестирования API с использованием **Python**, **Requests** и **Pytest**. Цель — показать структуру тестов, применяемые паттерны (Page Object), удобную генерацию отчётов с помощью **Allure CLI** и готовность репозитория к интеграции в CI.
