...
from tests.data.information.black_list.add.test_data import BLACK_LIST_ADD_TEST_DATA
...
@allure.story('POST /black_list.add')
class TestBlackListAdd:

    @pytest.mark.parametrize(
        'case',
        BLACK_LIST_ADD_TEST_DATA,
        ids=generate_test_ids(BLACK_LIST_ADD_TEST_DATA),
    )
    def test_black_list_add(
        self,
        case: GetCase,
        black_list_add_service: BlackListAddService,
        session_id: str,
    ) -> None:
        ...
        with allure.step(f'{case.description}'):
            response = black_list_add_service.post_black_list_add(
