...
from tests.data.information.black_list.delete.test_data import BLACK_LIST_DELETE_TEST_DATA
...
@allure.story('POST /black_list.delete')
class TestBlackListDelete:

    @pytest.mark.parametrize(
        'case',
        BLACK_LIST_DELETE_TEST_DATA,
        ids=generate_test_ids(BLACK_LIST_DELETE_TEST_DATA),
    )
    def test_black_list_delete(
        self,
        case: GetCase,
        black_list_delete_service: BlackListDeleteService,
        session_id: str,
    ) -> None:
        ...
        with allure.step(f'{case.description}'):
            response = black_list_delete_service.post_black_list_delete(
