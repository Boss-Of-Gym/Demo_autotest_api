...
from tests.data.information.black_list.fields.test_data import BLACK_LIST_FIELDS_TEST_DATA
...
@allure.story('GET /black_list.fields')
class TestBlackListFields:

    @pytest.mark.parametrize(
        'case',
        BLACK_LIST_FIELDS_TEST_DATA,
        ids=generate_test_ids(BLACK_LIST_FIELDS_TEST_DATA),
    )
    def test_black_list_fields(
        self,
        case: GetCase,
        black_list_fields_service: BlackListFieldsService,
        session_id: str,
    ) -> None:
        ...
        with allure.step(f'{case.description}'):
            response = black_list_fields_service.get_black_list_fields(session_id=session_id)
