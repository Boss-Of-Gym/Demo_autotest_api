...
from tests.data.information.black_list.toggle_block_status.test_data import BLACK_LIST_TOGGLE_BLOCK_STATUS_TEST_DATA
...
@allure.story('POST /black_list.toggle_block_status')
class TestBlackListToggleBlockStatus:

    @pytest.mark.parametrize(
        'case',
        BLACK_LIST_TOGGLE_BLOCK_STATUS_TEST_DATA,
        ids=generate_test_ids(BLACK_LIST_TOGGLE_BLOCK_STATUS_TEST_DATA),
    )
    def test_black_list_toggle_block_status(
        self,
        case: GetCase,
        black_list_toggle_block_status_service: BlackListToggleBlockStatusService,
        session_id: str,
    ) -> None:
        ...
        with allure.step(f'{case.description}'):
            response = black_list_toggle_block_status_service.post_black_list_toggle_block_status(
