...
from tests.data.information.clients_accounts.clients_accounts_fields.test_data import CLIENTS_ACCOUNTS_FIELDS_TEST_DATA
...
@allure.story('GET /clients_accounts.fields')
class TestClientsAccountsFields:

    @pytest.mark.parametrize(
        'case',
        CLIENTS_ACCOUNTS_FIELDS_TEST_DATA,
        ids=generate_test_ids(CLIENTS_ACCOUNTS_FIELDS_TEST_DATA),
    )
    def test_clients_accounts_fields(
        self,
        case: GetCase,
        clients_accounts_fields_service: ClientsAccountsFieldsService,
        session_id: str,
    ) -> None:
        ...
        with allure.step(f'{case.description}'):
            response = clients_accounts_fields_service.get_clients_accounts_fields(session_id=session_id)
