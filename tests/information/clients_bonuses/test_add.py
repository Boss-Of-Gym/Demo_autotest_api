...
from tests.data.information.clients_bonuses.clients_bonuses_add.test_data import CLIENTS_BONUSES_ADD_TEST_DATA
...
@allure.story('POST /clients_bonuses.add')
class TestClientsBonusesAdd:

    @pytest.mark.parametrize(
        'case',
        CLIENTS_BONUSES_ADD_TEST_DATA,
        ids=generate_test_ids(CLIENTS_BONUSES_ADD_TEST_DATA),
    )
    def test_clients_bonuses_add(
        self,
        case: GetCase,
        clients_bonuses_add_service: ClientsBonusesAddService,
        session_id: str,
    ) -> None:
        ...
        with allure.step(f'{case.description}'):
            response = clients_bonuses_add_service.post_clients_bonuses_add(
