...
from tests.data.information.clients_bonuses.clients_bonuses.test_data import CLIENTS_BONUSES_TEST_DATA
...
@allure.story('GET /clients_bonuses')
class TestClientsBonuses:

    @pytest.mark.parametrize(
        'case',
        CLIENTS_BONUSES_TEST_DATA,
        ids=generate_test_ids(CLIENTS_BONUSES_TEST_DATA),
    )
    def test_clients_bonuses(
        self,
        case: GetCase,
        clients_bonuses_service: ClientsBonusesService,
        session_id: str,
    ) -> None:
        ...
        with allure.step(f'{case.description}'):
            response = clients_bonuses_service.get_clients_bonuses(session_id=session_id)
