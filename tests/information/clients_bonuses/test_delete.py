...
from tests.data.information.clients_bonuses.clients_bonuses_delete.test_data import CLIENTS_BONUSES_DELETE_TEST_DATA
...
@allure.story('POST /clients_bonuses.delete')
class TestClientsBonusesDelete:

    @pytest.mark.parametrize(
        'case',
        CLIENTS_BONUSES_DELETE_TEST_DATA,
        ids=generate_test_ids(CLIENTS_BONUSES_DELETE_TEST_DATA),
    )
    def test_clients_bonuses_delete(
        self,
        case: GetCase,
        clients_bonuses_delete_service: ClientsBonusesDeleteService,
        session_id: str,
    ) -> None:
        ...
        with allure.step(f'{case.description}'):
            response = clients_bonuses_delete_service.post_clients_bonuses_delete(
