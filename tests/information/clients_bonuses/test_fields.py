...
from tests.data.information.clients_bonuses.clients_bonuses_fields.test_data import CLIENTS_BONUSES_FIELDS_TEST_DATA
...
@allure.epic('CRM API')
@allure.feature('Clients Bonuses')
@allure.story('GET /clients_bonuses.fields')
class TestClientsBonusesFields:

    @pytest.mark.parametrize(
        'case',
        CLIENTS_BONUSES_FIELDS_TEST_DATA,
        ids=generate_test_ids(CLIENTS_BONUSES_FIELDS_TEST_DATA),
    )
    def test_clients_bonuses_fields(
        self,
        case: GetCase,
        clients_bonuses_fields_service: ClientsBonusesFieldsService,
        session_id: str,
    ) -> None:
        ...
        with allure.step(f'{case.description}'):
            response = clients_bonuses_fields_service.get_clients_bonuses_fields(session_id=session_id)
